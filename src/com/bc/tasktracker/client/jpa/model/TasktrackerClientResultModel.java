/*
 * Copyright 2017 NUROX Ltd.
 *
 * Licensed under the NUROX Ltd Software License (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.looseboxes.com/legal/licenses/software.html
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.bc.tasktracker.client.jpa.model;

import com.bc.appbase.ui.PromptException;
import com.bc.appbase.ui.PromptUserUpdate;
import com.bc.appbase.ui.actions.ActionCommands;
import java.util.logging.Level;
import com.bc.tasktracker.client.TasktrackerApp;
import com.bc.tasktracker.jpa.model.TasktrackerCoreResultModel;
import java.util.Collections;
import java.util.List;
import java.util.Set;
import java.util.logging.Logger;

/**
 * @author Chinomso Bassey Ikwuagwu on Apr 8, 2017 1:17:28 PM
 */
public class TasktrackerClientResultModel<T> extends TasktrackerCoreResultModel<T> {

    private static final Logger logger = Logger.getLogger(TasktrackerClientResultModel.class.getName());

    public TasktrackerClientResultModel(
            TasktrackerApp app, Class<T> coreEntityType, 
            List<String> columnNames) {
        super(app, coreEntityType, columnNames, 
                new PromptUserUpdate(app.getUIContext()),
                new PromptException(app.getUIContext()));
    }
    
    @Override
    public boolean update(int row, int column, 
            Object entity, String entityColumn, Object entityValue, 
            Object target, String targetColumn, Object targetValue) {

        final boolean updated = super.update(row, column, 
                entity, entityColumn, entityValue, 
                target, targetColumn, targetValue);
        
        if(updated) {
            
            try{
                
                final TasktrackerApp app = this.getApp();
                
                app.getAction(ActionCommands.REFRESH_ALL_RESULTS).executeSilently(app, 
                        Collections.singletonMap(Set.class.getName(), Collections.singleton(target)));

                this.getApp().updateReports();
                
            }catch(RuntimeException e) {
                final String msg = "Error updating "+targetColumn;
                logger.log(Level.WARNING, msg, e);
                this.getApp().getUIContext().showErrorMessage(e, msg);
            }
        }
        
        return updated;
    }
    
    @Override
    public TasktrackerApp getApp() {
        return (TasktrackerApp)super.getApp(); 
    }
}
